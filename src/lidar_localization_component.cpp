...
void PCLLocalization::sendMapInChunks(
  const pcl::PointCloud<pcl::PointXYZI>::Ptr & map_cloud_ptr)
{
  if (!chunk_pub_) {
    RCLCPP_WARN(get_logger(), "chunk_pub_ is null, not sending chunks");
    return;
  }
...
void PCLLocalization::initializePubSub()
{
  ...
  path_pub_ = create_publisher<nav_msgs::msg::Path>(
    "path",
    rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());
  chunk_pub_ = create_publisher<lidar_localization_ros2::msg::MapChunk>(
      "map_chunks",
